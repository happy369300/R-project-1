...
# @param dataList 
# @param pnlList
# @return: xts with DailyPnL and CumPnL at portfolio level 

plotResults <- function(dataList, pnlList) {

	pfolioPnL <- xts(rep(0, nrow(dataList[[1]])), index(dataList[[1]])) # assumes indexes are all the same
	...
	pfolioSharpe <- sharpe(pfolioPnL$DailyPnL)
	chartSeries(pfolioPnL$CumPnL,theme='white',name=paste('Sharpe Ratio =',pfolioSharpe))

	sharpeList <- lapply(pnlList,function(x) sharpe(x$DailyPnL))

	for (i in 1:length(pnlList))
		# performance on individual series
		# addTA needs to be wrapped in plot; see https://stat.ethz.ch/pipermail/r-sig-finance/2009q3/004634.html
		plot(addTA(pnlList[[i]]$CumPnL,legend=paste('Series',i,': Sharpe Ratio = ',sharpeList[[i]]))) 

	return(list(pfoliosPnL=pfolioPnL,sharpeInd=max(unlist(sharpeList)),sharpeAgg=pfolioSharpe))
}

####################################################################################
# For simplicity we use the DailyPnL as a return to compute the Sharpe Ratio 
# The resulting Sharpe Ratio value is artificial BUT quite adequate for comparing 
# the performance of teams
#
# @param dailyPnL: xts of daily pnl which will be treated as simple returns
# @return: Sharpe ratio 

sharpe <- function(dailyPnL) {
	# we annualize the daily returns
	res <- mean(dailyPnL)*252/(sqrt(252)*apply(dailyPnL, 2, sd))
	s <- round(as.numeric(res),3) # only use three digits
	if (is.nan(s)) s <- 0
	return(s)
}
